...
import featured1 from '@/public/assets/featured/Featured-product-3.png'
import featured2 from '@/public/assets/featured/Featured-product-2.png'
import featured3 from '@/public/assets/featured/Featured-product-1.png'
    ...
    <main>
      {FeaturedData.map((loops) => {
        return (
          <div
            key={loops.id}
            className="relative flex flex-col lg:flex-row border-t-grey border-t-[0.5px] py-8 md:py-[60px] lg:even:flex-row-reverse first:border-t-0 lg:first:border-t last:border-b border-b-grey border-b-[0.5px]  lg:border-b-0"
          >
            <div className="lg:w-1/2 lg:first:mr-[37px] shrink-0 relative view-overlay">
              ...
              <Image src={loops.img} alt="featuredImage" width={100} height={100}  className="w-full" />
            </div>
...
function EllipseImage({ img }) {
  return (
    <div className="h-[42px] md:h-[51px] w-[42px] md:w-[51px] inline-block border border-[#78A3AD] rounded-full -ml-[21px] first:ml-0">
      <Image src={img} alt="EllipseImage" className="w-full h-full" />
    </div>
  );
}
